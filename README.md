...
36.数据结构
    ...
    -数据已经排好序：
        ...
        -插值查找：要求数据集排序且分布均匀，查找的时候midIndex= min + (key-arr[min])/(arr[max]-arr[min]) * (max-min)
...
90.日志技术：可以将日志信息写入文件或数据库中

体系结构：

        日志规范:大多为接口，规范日志的实现框架设计，如Commons Logging、Simple Logging Facade for Java
        实现结构: 例如Logback、Log4J、JUL等

Logback: 基于slf4j日志规范实现的框架，性能优于log4j。
需要配置xml文件（必须放置在src文件夹下），导入Jar包：

    logback-core: 核心jar包，为其他模块提供基础代码
    logback-classic: 完整实现slf4j API模块
    logback-access: 与Tomcat、Jetty等servlet容器集成，以提供HTTP访问日志功能。

配置文件:
    
    -通过<appender>标签来选择写入的位置: CONSOLE:控制台；FILE：文件
     在<appender>标签中可以进行输出格式、输出字符、文件地址等配置

    -日志级别: 控制日志信息的输出
     当日志信息的级别大于等于设置的日志级别，才会输出。

91.类加载器：将存储在硬盘上的文件(如class)加载到内存中(如JVM)
    
    加载时机:用到就加载，不用不加载。
    例如:调用静态变量、静态方法、创建子类对象、创建该类对象等等

    加载过程：加载->验证->准备->解析->初始化 

    加载:（通过包名+类名找到）将类的字节码文件从硬盘加载到内存中，根据类中的方法和变量，生成该类对应的class文件，并存储在内存中。
    
    验证->准备->解析: 链接过程
        -验证：检查class文件的信息是否符合虚拟机规范，有无安全隐患。
        -准备：将class文件中静态变量初始化。
        -解析：将二进制数据流中的符号引用解析为直接引用--找到本类所使用的其他类，并关联对应的类。
    
    初始化:根据程序员的规划，为静态变量赋值，并初始化其他资源。

类加载器的分类:<img height="250" src="img/类加载器的分类.png" width="650"/>，什么类该用什么加载器由虚拟机内部规定好了。

双亲委派模型:除了顶层的启动类加载器，所有加载器都应该有"双亲":<img height="450" src="img/双亲委派模型.png" width="450"/>

在这个模型中的继承关系是逻辑上的继承: 当某个加载器得到class文件后会向上传递至启动类加载器，如果其无法加载这个文件，就会向下传递至能加载这个文件的加载器。

获取某个文件的字节流：InputStream classLoader.getResourceAsStream("相对于当前src目录的路径")。

92.XML(配置文件之一，可扩展的标记语言)：当数据较多，成组出现时，可以使用；但缺点是解析复杂。
    
    标记语言: 使用标签来描述数据。
    可扩展: 可以自定义标签内容。
    XML的作用:1.存储数据与传输数据；2.作为配置文件。

XML标签规则:
    
    第一行必须是<?XML version="1.0" encoding="UTF-8"?>
    1.必须有且只有一个根标签(最外层必须有一个标签)
    2.标签标识符必须符合规范与约束
    3.标签要成对出现(除了某些特殊的标签)
    4.标签中可以带有属性<student id="1">;
    5.特殊字符的表示方式
        -使用规定的字符表示。
        -在<![CDATA[内容]]>中，所有内容都被视为普通内容。
    6.注释: <!--内容-->

<img height="200" src="img/XML特殊字符.png" width="342"/>

约束:XML文件的格式要符合某种规范，使得其内容能被程序正确地读取

    约束一:DTD约束：在dtd文件中规定标签的标识符以及其嵌套关系，但无法约束数据类型。
    约束二:schema约束：schema本身也是一个xml文件(后缀为.xsd)，同时也受到其他文件的约束。
                    -schema是一种较为严格的约束。

解析:1.SAX技术:逐行扫描读取。但是无法修改xml文件，优点是不占内存，但随着硬件技术发展，优点已经不占优势了
    2.DOM:直接加载一整个XML文件，形成树形结构，可以进行CRUD，缺点是当XML文件较大时，占内存，但随着硬件技术发展，已经不是什么大问题了。

DOM：将整个XML文件解析为Document，其中包含node。
    
    node中包含:1.Element(标签); 2.Attribute(属性); 3.Text(文本内容)。
    创建DOM对象，使用SAX逐行读取整个文件，然后逐层获取Node，从Node中获取Element、Attribute，以及其中的Text

Xpath：快速检索。依赖于DOM的第三方jar包
    
    1.selectSingleNode("路径");
    2.selectNodes("路径");
    路径的四种表示：
    /:单级路径
    //:多级路径
    @:属性值
        1.绝对路径：由document调用方法: /根节点/子节点/子节点......, 最终返回最后一级节点，该节点的嵌套关系严格遵循路径格式。
        2.相对路径：由当前节点调用方法: ./子节点/......, ./表示从当前节点开始
        3.全文查找: //contact：无论contact在哪，都返回。
                  //contact/element: element必须是contact的子节点。返回子节点。
                  //contact//element: element是contact的子节点/孙节点，返回element。
        4.属性检索: @属性名: 全文查找该属性并返回.
                  //标签[@属性]: 全文查找并返回带有该属性的标签。
                  //标签[@属性='value']: 全文查找带有该属性的并且值为value的标签。 

93.单元测试:针对Java中最小的功能单元(方法)进行测试
使用JUnit:为Java提供测试方法的jar包

    1.被测试的方法必须: 公共，无参，无返回值，非静态。
    2.在方法名上加上注解@Test。
    3.被测试的方法报错也不会影响其他方法的测试结果。

进行单元测试时不能污染原数据，测试完毕后不能增加数据，减少数据，修改数据。
    
    一般会新建测试类来进行单元测试，在这个类中没有main方法。
    1.使用@Before：每个@Test测试方法执行前都会执行一次，用于方法测试前的准备工作，比如原数据的备份等。
    2.使用@After：每个@Test测试方法执行后都会执行一次。用于方法测试过后的整理工作，比如还原数据，删除多余数据等。
    3.上述方法有对应的静态注释，表示所有单元方法执行前/后，只执行一次。
    4.使用断言Assert判断方法测试是否通过。

扩展：在单元测试中，相对路径是相对于当前模块(与src文件夹同级)，而不是当前项目。
    
94.注解：提供给JVM的提示信息，使其为我们提供特殊服务。
    
常见注解: 
        
    @Override: 提示重写方法，检查格式是否正确。
    @Deprecated: 提示方法过时，调用时有横线。
    @SuppressWarning("all"):all表示全部。 压制警告，表示不提示警告，比如单词错误，变量未被调用等。
    @Test: 提示测试方法，可以对此方法单独测试。
    @Before\@After......

自定义注解:

    格式 public @interface 注解名{ 
        public 类别 属性名() [default value];  //默认值: 引用注解时必须属性值赋值，没有赋值就使用默认值。  
        ......    
        }

元注解: 用来注解别的注解的一种注解

    @Target：该注解可以使用的范围(成员变量、成员方法、构造方法、类......)
    @Retention：该注解的生命周期(到什么阶段会消失，什么阶段依然存在)

<img height="300" src="img/元注解的常用值.png" width="450"/>

注解解析:将注解和反射结合使用，判断类以及成员是否被注解了，并获取注解，分析其内容。
<img height="300" src="img/注解解析.png" width="600"/>

_**2025.4.4完成黑马程序员Java基础所有课程学习!**_